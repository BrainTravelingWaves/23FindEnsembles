function [] = CreateChMEGmatrix(MM,m,t0,dTime,mode)
%% CreateChMEGmatrix
Nchn=306;
%dTime=1000;
%treshC=0.7;
...
if MM==1 
    ...
    Nwrd=5;
    wrd=wrd1;
    stimulus=m.Events(1).times; % Start word
    %behavir=m1.Events(2).times(1,4:end); % Reaction
    timeSig=m.Time;     % Signal time
    sig=m.F(1:Nchn,:);  % Signal of MEG
end
%%
if MM==2 
    Nstr=8;
...
for ns=1:Nstr % 1-8
    for nw=1:Nwrd % 1-5
        %% MEG1+GRD2+GRD3
        for i=1:Nchn
            sigMGG(i,:)=sig(i,tTime(ns,nw)+1+t0:tTime(ns,nw)+dTime);
        end
        %% cor_MGG_MGG
        if mode==1
        for i=1:Nchn
